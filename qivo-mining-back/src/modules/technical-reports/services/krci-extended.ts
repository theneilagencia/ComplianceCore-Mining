...
export const KRCI_RULES: KRCIRule[] = [
  ...
  // ============================================================
  // SEC S-K 1300 SPECIFIC RULES (10 rules) - U.S. SEC requirements
  // ============================================================
  
  {
    ...
    message: 'Qualified Person não especificado (Item 1302(d) obrigatório)',
    weight: 25,
    severity: 'critical',
    mode: 'light',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && !r.qualifiedPersonName,
    recommendation: 'SEC S-K 1300 requer Qualified Person com credenciais P.E., P.Geo., CPG, ou FAusIMM',
  },
  {
    code: 'KRCI-SEC002',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Credenciais do Qualified Person não especificadas',
    weight: 20,
    severity: 'critical',
    mode: 'light',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.qualifiedPersonName && !r.qualifiedPersonCredentials,
    recommendation: 'Especificar credenciais profissionais (P.E., P.Geo., CPG, etc.) e número de registro',
  },
  {
    code: 'KRCI-SEC003',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Experiência relevante do QP não documentada (mínimo 5 anos)',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.qualifiedPersonName && !r.qualifiedPersonExperience,
    recommendation: 'Documentar pelo menos 5 anos de experiência relevante do Qualified Person',
  },
  {
    code: 'KRCI-SEC004',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'NPV não especificado (Item 1310 obrigatório)',
    weight: 22,
    severity: 'critical',
    mode: 'light',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.hasReserves === 'yes' && !r.npv,
    recommendation: 'Item 1310 requer análise econômica completa com NPV após impostos',
  },
  {
    code: 'KRCI-SEC005',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'IRR não especificado (Item 1310 obrigatório)',
    weight: 22,
    severity: 'critical',
    mode: 'light',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.hasReserves === 'yes' && !r.irr,
    recommendation: 'Item 1310 requer Taxa Interna de Retorno (IRR) após impostos',
  },
  {
    code: 'KRCI-SEC006',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Taxa de desconto não especificada',
    weight: 15,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.npv && !r.discountRate,
    recommendation: 'Especificar taxa de desconto utilizada no cálculo do NPV',
  },
  {
    code: 'KRCI-SEC007',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Modifying factors incompletos (Item 1307)',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && (!r.miningRecovery || !r.processingRecovery || !r.dilution),
    recommendation: 'Item 1307 requer fatores de recuperação mineração/processamento e diluição',
  },
  {
    code: 'KRCI-SEC008',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Análise de sensibilidade não fornecida (Item 1310)',
    weight: 16,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.npv && !r.sensitivityAnalysis,
    recommendation: 'Item 1310 requer análise de sensibilidade (preço, teor, custos)',
  },
  {
    code: 'KRCI-SEC009',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Bond de recuperação ambiental não especificado (Item 1305)',
    weight: 14,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && !r.reclamationBond,
    recommendation: 'Item 1305 requer valor do bond ou garantia financeira para recuperação',
  },
  {
    code: 'KRCI-SEC010',
    category: 'norma',
    section: 'SEC S-K 1300',
    message: 'Método de estimativa de recursos não documentado (Item 1306)',
    weight: 17,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.standard === 'SEC_SK_1300' && r.resourceIndicated && !r.estimationMethod,
    recommendation: 'Item 1306 requer documentação detalhada do método de estimativa (kriging, ID, etc.)',
  },

  // ============================================================
  // BRAZILIAN COMPLIANCE RULES (20 rules) - Conformidade Regulatória BR
  // ============================================================
  
  {
    code: 'KRCI-BR001',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Processo ANM não informado',
    weight: 25,
    severity: 'critical',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && !r.anm_processNumber,
    recommendation: 'Projetos no Brasil devem informar o número do processo ANM (formato: XXXXX.XXXXXX/XXXX)',
  },
  {
    code: 'KRCI-BR002',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Fase do processo ANM não especificada',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.anm_processNumber && !r.anm_processPhase,
    recommendation: 'Especificar fase: Autorização de Pesquisa, Concessão de Lavra, Licenciamento, etc.',
  },
  {
    code: 'KRCI-BR003',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'CFEM não calculada',
    weight: 20,
    severity: 'high',
    mode: 'full',
    check: (r) => r.anm_processPhase === 'CONCESSAO_LAVRA' && !r.anm_cfemValue,
    recommendation: 'Concessões de lavra devem calcular CFEM (Compensação Financeira pela Exploração Mineral)',
  },
  {
    code: 'KRCI-BR004',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Alíquota CFEM não informada',
    weight: 15,
    severity: 'high',
    mode: 'full',
    check: (r) => r.anm_cfemValue && !r.anm_cfemPercentage,
    recommendation: 'Informar alíquota CFEM conforme Lei 13.540/2017 (0,2% a 4% conforme substância)',
  },
  {
    code: 'KRCI-BR005',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Licença ambiental não informada',
    weight: 25,
    severity: 'critical',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && r.anm_processPhase === 'CONCESSAO_LAVRA' && !r.ibama_licenseNumber && !r.state_environmentalLicense,
    recommendation: 'Concessões de lavra requerem Licença Ambiental (IBAMA ou órgão estadual)',
  },
  {
    code: 'KRCI-BR006',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Tipo de licença IBAMA não especificado',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.ibama_licenseNumber && !r.ibama_licenseType,
    recommendation: 'Especificar tipo de licença: LP (Prévia), LI (Instalação), LO (Operação)',
  },
  {
    code: 'KRCI-BR007',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Validade da licença IBAMA não informada',
    weight: 16,
    severity: 'high',
    mode: 'full',
    check: (r) => r.ibama_licenseNumber && !r.ibama_licenseValidity,
    recommendation: 'Informar data de validade da licença ambiental',
  },
  {
    code: 'KRCI-BR008',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Estudo ambiental não especificado',
    weight: 20,
    severity: 'high',
    mode: 'full',
    check: (r) => r.ibama_licenseNumber && !r.ibama_environmentalStudy,
    recommendation: 'Especificar estudo ambiental: EIA/RIMA, RCA, PCA, PRAD, EAS',
  },
  {
    code: 'KRCI-BR009',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Compensação ambiental não calculada',
    weight: 14,
    severity: 'medium',
    mode: 'full',
    check: (r) => r.ibama_environmentalStudy === 'EIA_RIMA' && !r.ibama_compensationValue,
    recommendation: 'EIA/RIMA requer compensação ambiental (mínimo 0,5% do custo total - Lei 9.985/2000)',
  },
  {
    code: 'KRCI-BR010',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Outorga de água não informada',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && r.anm_processPhase === 'CONCESSAO_LAVRA' && !r.ana_waterGrantNumber,
    recommendation: 'Mineração requer outorga de direito de uso de recursos hídricos (ANA ou órgão estadual)',
  },
  {
    code: 'KRCI-BR011',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Vazão outorgada não especificada',
    weight: 12,
    severity: 'medium',
    mode: 'full',
    check: (r) => r.ana_waterGrantNumber && !r.ana_waterVolume_m3h,
    recommendation: 'Informar vazão outorgada em m³/h',
  },
  {
    code: 'KRCI-BR012',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Proximidade de Terra Indígena não avaliada',
    weight: 22,
    severity: 'critical',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && !r.funai_indigenousLandProximity,
    recommendation: 'Avaliar proximidade com Terras Indígenas (Convenção 169 OIT - consulta prévia obrigatória)',
  },
  {
    code: 'KRCI-BR013',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Consulta prévia FUNAI não realizada',
    weight: 25,
    severity: 'critical',
    mode: 'full',
    check: (r) => (r.funai_indigenousLandProximity === 'SOBREPOSTA' || r.funai_indigenousLandProximity === 'LIMITROFE') && 
                   (!r.funai_consultationStatus || r.funai_consultationStatus === 'NAO_INICIADA'),
    recommendation: 'Projetos próximos/sobrepostos a TIs requerem consulta prévia, livre e informada (Convenção 169 OIT)',
  },
  {
    code: 'KRCI-BR014',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Proximidade de território quilombola não avaliada',
    weight: 18,
    severity: 'high',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && !r.quilombola_proximityStatus,
    recommendation: 'Avaliar proximidade com territórios quilombolas certificados (Fundação Palmares)',
  },
  {
    code: 'KRCI-BR015',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Província geológica CPRM não identificada',
    weight: 10,
    severity: 'medium',
    mode: 'deep',
    check: (r) => r.metadata?.country === 'BR' && !r.cprm_geologicalProvince,
    recommendation: 'Identificar província geológica conforme mapeamento CPRM/SGB',
  },
  {
    code: 'KRCI-BR016',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Barragem de rejeitos sem categoria de risco',
    weight: 25,
    severity: 'critical',
    mode: 'full',
    check: (r) => r.dam_exists === 'SIM' && !r.dam_riskCategory,
    recommendation: 'Barragens devem ter Categoria de Risco (CRI) conforme Portaria ANM 95.344/2022',
  },
  {
    code: 'KRCI-BR017',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Barragem sem Dano Potencial Associado (DPA)',
    weight: 22,
    severity: 'critical',
    mode: 'full',
    check: (r) => r.dam_exists === 'SIM' && !r.dam_damageCategory,
    recommendation: 'Classificar Dano Potencial Associado: Baixo, Médio ou Alto',
  },
  {
    code: 'KRCI-BR018',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'Barragem não cadastrada no SIGBM',
    weight: 20,
    severity: 'high',
    mode: 'full',
    check: (r) => r.dam_exists === 'SIM' && !r.dam_sigbmNumber,
    recommendation: 'Cadastrar barragem no Sistema de Gestão de Barragens da ANM (SIGBM)',
  },
  {
    code: 'KRCI-BR019',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'TAC não declarado',
    weight: 12,
    severity: 'medium',
    mode: 'full',
    check: (r) => r.metadata?.country === 'BR' && !r.tac_exists,
    recommendation: 'Declarar existência ou não de Termo de Ajustamento de Conduta (TAC)',
  },
  {
    code: 'KRCI-BR020',
    category: 'norma',
    section: 'Conformidade Brasil',
    message: 'PAE não protocolado para Concessão de Lavra',
    weight: 22,
    severity: 'high',
    mode: 'full',
    check: (r) => r.anm_processPhase === 'CONCESSAO_LAVRA' && 
                   (!r.anm_paeStatus || r.anm_paeStatus === 'NAO_APLICAVEL' || r.anm_paeStatus === 'EM_ELABORACAO'),
    recommendation: 'Concessão de Lavra requer PAE (Plano de Aproveitamento Econômico) protocolado e aprovado',
  },
