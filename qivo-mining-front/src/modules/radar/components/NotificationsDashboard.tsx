...
export default function NotificationsDashboard({ darkMode = false }: NotificationsDashboardProps) {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/radar/notifications?${params.toString()}`, {
        credentials: 'include',
      });
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/radar/notifications/${notificationId}/read`, {
        method: 'PATCH',
        credentials: 'include',
      });
      const data = await response.json();
